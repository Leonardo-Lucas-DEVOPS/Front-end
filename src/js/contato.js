...
// Máscara de Número (apenas números)
function mascaraNumero(event) {
    let valor = event.target.value.replace(/\D/g, '');
    event.target.value = valor;
}

// Máscara de E-mail (minúsculo, sem espaços)
...
async function enviarFormulario(event) {
    ...
    try {
        ...
        if (result.result === 'success') {
            console.log('✅ Dados salvos na linha:', result.row);
...
function inicializarFormulario() {
    ...
    console.log('✅ Formulário de contato inicializado (Google Sheets)');
...
const observador = new MutationObserver((mutations) => {
    ...
    if (form && !formularioInicializado) {
        console.log('📝 Detectado formulário de contato, inicializando...');
